...
lumen/
  ...
  experiments/<experiment>/            experiment.yaml: one question over a dataset; experiment.md: notes, failure
                                       ...
                                       flows/<flow>/: a flow only this experiment runs; nodes/<node>/ and
                                       prompts/<name>.md: what its variants put in place of a node or a prompt;
...
| Long or noisy input | on `length: very_long` the right answer drops below X; condensing first beats one step | `threshold` with `cases.tags` on one level; `compare` of two flows in one slot (`varies: what: flow`) |
...
| Judge reliability | judge J catches more than X of planted defects and passes more than Y of clean answers | a local flow that runs the judge on clean answers and copies with one planted defect, `expected` on the label (the experiment `critique_planted_defects`) |
...
| Stability | a share of cases passes only sometimes | `repeats: 3` or more, read `stability` and `pass_k`; an A/A pair (two variants that both keep the flow as written, no `varies`; `compare`, `margin: 0`) gives the noise floor (the experiment `panel_aa_noise`) |
| Agent per step | agent B on step S is not worse than A by more than m, and a pass costs less | `noninferior` on the range `from: S, to: S`, `varies: what: agent` on S, guardrails `cost_of_pass`, `schema_valid_first_try`, `latency_p95_ms`; downgrade one step at a time |
| Prompt | prompt P on step S beats the prompt as written by more than m | `varies: what: prompt` on S, the text in `prompts/<name>.md`; the inputs and output schema stay, so only the wording differs |
| Step logic | algorithm A of a `code` step (a merge rule, a vote, a threshold) is not worse than the one as written | `varies: what: use` on the step, each algorithm an alternative node in `nodes/`; the ways are variants, never checks |
| Split a step | a chain S1 → S2 beats one call by more than m, and not just by calling more | `varies: what: flow` on a slot, a `call` node, with the single step, the chain and a flow of equal budget (a `parallel` of k identical `llm` branches and a `code` majority vote) as local flows; guardrail `cost_of_pass` `relative: true` |

### Writing an experiment

An experiment is `experiments/<experiment_id>/experiment.yaml`; its id is the folder name. `lumen/experiments/` has
one for every question kind and every kind of factor: start from the closest.
...
- `subject` — what runs: `flow: <flow_id>`, optionally narrowed to a range of top-level nodes with `from` and `to`
  (the nodes above the range replay the case `node_outputs`). The flow is a local flow of the experiment, looked up
  first in `experiments/<experiment_id>/flows/<flow_id>/` (`flow.yaml` with its nodes, or `flow.py`), or a flow of the
  project. A local flow runs only in this experiment; test a structure there before you build it into the project.
- `varies` — the one factor the variants change: `what` and `nodes`, the nodes of the subject it touches, each named
  by its own id (`revise`, not `polish__revise`), inside the range when there is one. Required with two or more
  variants.
- `cases` — `dataset: <dataset_id>`, optionally `tags: {<dimension>: <value>}`: a case is selected when every listed
  tag has that value.
- `variants` — at least one. A variant is an `id` and `nodes: {<node>: <value>}` for nodes of `varies.nodes`; a variant
  with only an `id` is the subject as written, and a factor node a variant leaves out runs as written. A variant never
  names a bare model: a new candidate model is a new `agents/<agent>.yaml` with its settings and output mode.
...
**One factor per experiment: the compared things are variants, the measures are checks.** Each way of doing a step is a
variant, a row of the series; what tells a good result from a bad one is a check, a column. Never write the ways as
checks over one variant (three merge rules as three checks): make each a variant through the factor that changes it.
A variant that changes two things at once cannot say which one made the difference; a combination you want to test as
one thing (a cheaper agent with a prompt tuned for it) goes into one alternative node and is compared with `use`.

| `what` | The node | A variant's value | Files |
|---|---|---|---|
| `agent` | an `llm` node | an agent of the project | `agents/<agent>.yaml` |
| `prompt` | an `llm` node | the name of a prompt text; it replaces the prompt of that node only, and the inputs, output schema and checks of its inference stay | `experiments/<experiment_id>/prompts/<name>.md` |
| `use` | any node | the id of an alternative node; it runs under the id of the node it replaces, so `$<node>.out` bindings keep working, and its children are found among the alternatives first | `experiments/<experiment_id>/nodes/<alternative>/`: `<alternative>.node.yaml` and its `.py`, `.inference.yaml`, `.prompt.md` |
| `flow` | a `call` node | a flow with the input and output types of the flow the node calls as written; the node keeps its input bindings | `experiments/<experiment_id>/flows/<flow_id>/`, else a flow of the project |

```
experiments/<experiment_id>/
  experiment.yaml             subject, varies, cases, variants, checks, question, plan
  experiment.md               notes, failure modes, decisions
  checks.py                   check functions of this experiment
  flows/<flow_id>/            local flows: flow.yaml with nodes/, or flow.py
  nodes/<alternative>/        alternative nodes for use
  prompts/<name>.md           prompt texts for prompt
  findings/<series>.yaml      findings the server wrote, never edit
```

To compare whole structures, give the subject a slot: a local flow with one `call` node that passes its input on,
and each structure a local flow the variants plug into that node with `flow`. `aqven check` assembles and compiles
every variant; an error inside an alternative, a prompt text or a local flow is reported at
`variants[i].nodes.<node>` and names the file it comes from. In `lumen/experiments/`, `judge_panel_agents` varies an
agent, `panel_judge_prompt` a prompt on three nodes, `panel_merge_rule` a code step, and `panel_single_judge`,
`intent_split_long_messages` and `intent_ballot_pair` the flow in a slot.

### Cases and checks

- A dataset is `datasets/<dataset_id>.yaml` with `flow: <flow_id>`; a local flow subject also runs a dataset without
  `flow`, or the dataset of a project flow with the same input type. A case has a unique `name`, `inputs`, and as
  needed `expected_output`, `node_outputs` (outputs of the top-level nodes a range skips) and `tags`.
...
| a prompt wins | copy the text of `prompts/<name>.md` into the node's prompt file and preview it with `aqven prompt preview` |
| an alternative node or a local flow wins | move its nodes into the project flow with `flow_patch`; there is no promote tool |
...
(that is a new experiment); vary two factors in one experiment; write the compared ways as checks instead of
variants; compare a multi-call flow only against a single call; use a judge where `regex` or code can check; weaken
a type or a limit so a series passes; present a `dev` number as a finding.
...
Experiments and datasets: `E_FLOW_UNKNOWN`, `E_RANGE_INVALID` (a `from`/`to` that is not a range of top-level nodes),
`E_FACTOR_MISSING` (several variants and no `varies`), `E_FACTOR_NODE_UNKNOWN` (a `varies.nodes` entry that is not a
node of the subject or lies outside the range), `E_FACTOR_KIND` (`agent` or `prompt` on a node that is not `llm`,
`flow` on one that is not `call`), `E_VARIANT_OUTSIDE_FACTOR` (a variant sets a node not in `varies.nodes`),
`E_ALTERNATIVE_UNKNOWN`, `E_ALTERNATIVE_ID_TAKEN` (an alternative with the id of a subject node), `E_PROMPT_MISSING`
(a variant names a prompt with no file in `prompts/`), `E_FACTOR_FLOW_CONTRACT` (a flow for the slot with other input
or output types), `E_VARIANT_INVALID` (a question names a variant that is not declared), `E_AGENT_UNKNOWN`,
`E_DATASET_UNKNOWN`, `E_DATASET_MISMATCH` (the dataset feeds another flow, or a local flow's input type differs),
`E_CASES_EMPTY` (the tag filter selects nothing), `E_CASE_DUPLICATE`, `E_EXPECTED_MISSING` (an `expected` check on a
case without `expected_output`), `E_CHECK_PATH_UNKNOWN`, `E_METRIC_UNKNOWN`, `E_EXPERIMENT_UNKNOWN` (a `validated_by`
that does not exist), `E_ID_DUPLICATE`, `W_PLAN_EXCEEDS_CASES`, `W_CHECK_CONTEXT_MISMATCH`, `W_JUDGE_INPUT_UNBOUND`,
`E_FINDING_TAMPERED`, `W_FINDINGS_STALE`, `W_VARIANT_DUPLICATE` (two variants set the same values),
`W_ALTERNATIVE_UNUSED` (an alternative, a prompt text or a local flow no variant uses).
...
| `uv run aqven tree lumen` | entities by kind with their files, experiments with their local flows and alternatives included |
