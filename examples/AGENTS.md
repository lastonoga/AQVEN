...
- Write one free-text note per failing trace about its first failure, then group the notes into failure modes: an id,
  a one-line definition, a count, two or three `run_id`s. Keep them under "Failure modes" in the `experiment.md` of
  the look experiment. The id becomes the `failure_mode` of every experiment that tests it, and the section of
  `FINDINGS.md` its findings land in. Stop grouping when about 20 more failing traces add no new mode; few failures
  means you need harder cases, not less reading.
...
- Rank the modes by count, then by harm, and test the riskiest first. Never write hypotheses from a generic list
  ("hallucination", "toxicity") before you have read traces.
...
guard is in place; or the agreed budget is spent. Stop and ask the developer when a series awaits approval, when
several variants trade quality against cost, or when "done" turns out not to be measurable.
