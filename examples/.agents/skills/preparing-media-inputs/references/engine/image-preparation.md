# How to prepare images for a flow

Keep photos at their native resolution, apply the EXIF orientation, send crops as Image[], look at a contact sheet and at synthetic images before a series, and confirm in the trace what the model received.

## Contents

- [When you need this](#when-you-need-this)
- [What AQVEN does with an image](#what-aqven-does-with-an-image)
- [Steps](#steps)
  - [Example](#example)
- [See also](#see-also)
...
- An `Image` value is a blob: AQVEN stores the bytes you upload and sends those bytes to the provider unchanged.
  ...
  sees it; [media has real limits](../concepts/media-has-real-limits-on-both-sides.md) lists what each provider does.
- An inference attaches its own top-level media inputs only. Each item of an `Image[]` input is its own
  attachment, in list order. An `Image` inside a record input is not attached, and `aqven check` does not
...
9. **Check what the model received.** Before a run, `uv run aqven prompt preview <flow>.<node>` lists
...
- Tested snippets — a flow with an `Image[]` input and a dataset case with a photo.
- [Media and dynamic values](../reference/media.md) — the fields of a media value.
