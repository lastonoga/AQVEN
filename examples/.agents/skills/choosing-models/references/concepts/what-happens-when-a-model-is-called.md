...
[`on_rate_limit`](../integrations/model-providers.md#when-a-provider-rate-limits) decides how long it waits and
...
configured](../engine/check-providers.md) for what turning `strict: true` on actually requires.
...
- [How to check your model providers are configured](../engine/check-providers.md) — what `output.strict`
  does instead of leaning on this retry, and what `aqven check` requires before it compiles.
- [How to find a model's real structural limits](../engine/check-shapes.md) — the repair retry can fix a
