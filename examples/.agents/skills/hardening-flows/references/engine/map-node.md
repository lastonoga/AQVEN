...
- How to write a step in Python — what `summarize`, this example's body, actually
  is.
- The engineering loop — what to do when a run's output isn't what you
  expected.
- Node specifications — every field on `MapNodeSpec`, generated from the code.
- [Built-in policies and evaluators](../reference/built-in-policies.md) — every `on_item_error` policy's
