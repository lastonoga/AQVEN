...
- [Steps](#steps)
  - [Example](#example)
- [Shaping the out fields](#shaping-the-out-fields)
...
- Write `<stem>.inference.yaml`: the `in` fields the model reads and the `out` fields it must return,
  ...
  how to constrain a field's values for what each one does, with its own
  example.
...
An `llm` node's model call runs on Pydantic AI, with a fixed layer
of guarantees AQVEN adds on top of every call in the project. See that page for what AQVEN takes as-is
and what it adds.

## See also

- How to constrain a field's values — every constraint keyword an `in` or
...
- How to find a model's real structural limits — before shaping `out` fields
  below with real nesting, find how deep this agent's model actually holds up.
- [Node specifications](../reference/nodes.md) — every field on `LlmNodeSpec`, generated from the code.
