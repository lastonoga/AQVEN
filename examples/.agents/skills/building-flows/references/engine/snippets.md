...
Copy-ready project files for the common tasks — a flow, llm, code, map, parallel, switch and call steps, prompt variants and fragments, a dataset, one experiment per factor kind, rate limits and the spend cap — every one of them passes aqven check.

## Contents

- [When you need this](#when-you-need-this)
- [How these snippets stay correct](#how-these-snippets-stay-correct)
- [Project file: provider, rate limits, spend cap](#project-file-provider-rate-limits-spend-cap)
- [Agents](#agents)
- [Types](#types)
- [A flow](#a-flow)
- [A step in Python](#a-step-in-python)
- [An llm step with photos, a prompt variant and a fragment](#an-llm-step-with-photos-a-prompt-variant-and-a-fragment)
- [A map with on_item_error](#a-map-with-on_item_error)
...
- [A ULID for client_op_id](#a-ulid-for-client_op_id)
- [See also](#see-also)
...
working project. Copy the block that matches your task, rename the ids, and run `aqven check`.
...
test runs `aqven check` on the project and fails on any error or warning. The title of each block is the
file path from the package root, the folder that holds `aqven.yaml`.

- `types.py` is not shown: `aqven generate` (or `aqven check`) writes it from the YAML.
- The names of generated classes, such as `ListingReviewPrepareOut`, come from that file. Read them with
  `aqven tree` instead of guessing them.
...
read or produce: a live probe proves it (`aqven models check <agent> --project . --live`).
...
```bash
uv run aqven prompt preview listing_review.match_photos --project . --variant category_guide=default
```
...
`aqven check` names the file and line of each break.
...
- [How to write a prompt](prompts.md) — prompt levels, variant slots and fragments in detail.
- [Patterns in the showcase](lumen-patterns.md) — the same patterns in the showcase project, file by file.
- [How to run an experiment](experiments.md) — questions, checks and the factor.
- How to connect a model provider — rate limits per model.
- How to choose models on OpenRouter — catalogue fields and
  `provider_options`.
- How to prepare images for a flow — crops, orientation, contact sheets.
- [Diagnostics and error codes](../reference/diagnostics.md) — every code `aqven check` reports.
