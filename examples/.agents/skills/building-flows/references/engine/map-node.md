...
- [How to branch into parallel steps](parallel-node.md) — fixed branches that all run at once,
  instead of one body node run per list item.
- [How to write a step in Python](code-node.md) — what `summarize`, this example's body, actually
  is.
- The engineering loop — what to do when a run's output isn't what you
  expected.
- [Node specifications](../reference/nodes.md) — every field on `MapNodeSpec`, generated from the code.
- Built-in policies and evaluators — every `on_item_error` policy's
