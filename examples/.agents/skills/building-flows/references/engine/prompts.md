...
Write a prompt as plain text, a Liquid template, or a Python function — AQVEN picks the level from what the file actually contains.
...
- [Steps](#steps)
  - [Example](#example)
- [Variant slots and fragments](#variant-slots-and-fragments)
  - [Fragments](#fragments)
...
- `aqven check` parses every prompt file and rejects one that doesn't parse, is missing, or
...
- `default` is the variant for any other value, and for `null` when the input is optional. Without `default`,
  `cases` has to cover every value of the enum, and the input cannot be optional: `aqven check` reports
...
```bash
uv run aqven prompt preview support_case.revise --project . --variant lamp_guide=rechargeable
```
...
run an experiment that changes the prompt instead: see [How to run an experiment](experiments.md).
...
Level-2 templates render on python-liquid — real Liquid syntax, not
...
- [How to call a model](llm-node.md) — the node kind that calls the prompt you write here.
- [How to customize a node's display in Studio](display-templates.md) — the same file-naming and
  Liquid pattern, for how the node's input and output look in Studio instead of what the model reads.
- [Three prompt levels](../concepts/three-prompt-levels.md) — when to reach for level 2 or level 3 instead
  of staying at plain text, and what each one costs you.
- [Inference specifications](../reference/inference.md) — every field on `InferenceSpec`, including `prompt`.
- [Rendered prompts](../reference/prompts.md) — the `RenderedPrompt` and `PromptMessage` types a level-3
  function returns.
