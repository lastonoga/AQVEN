...
- How to connect AQVEN as an MCP server — getting an agent connected in the
  first place.
- How to check and test a project as an agent — run `aqven_check` after a
  patch to confirm the result the same way a human would before committing.
- [Two ways to change a project](../concepts/two-ways-to-change-a-project.md) — why `flow_patch`'s
