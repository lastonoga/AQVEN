...
When to reach for a plain prompt, a Liquid template, or a Python function, and what each one gives up in exchange for what it gains.

## Contents

- [In short](#in-short)
- [Level 1: plain text is enough until wording has to change](#level-1-plain-text-is-enough-until-wording-has-to-change)
- [Level 2: branch on an input, with a checked safety net](#level-2-branch-on-an-input-with-a-checked-safety-net)
- [Level 2's ceiling: what it won't let you write](#level-2s-ceiling-what-it-wont-let-you-write)
- [Level 3: a Python function, and what it gives up](#level-3-a-python-function-and-what-it-gives-up)
...
`aqven check` will find for you.

## See also

- [How to write a prompt](../engine/prompts.md) — file naming, how AQVEN detects the level, the
  `{% message %}` tag, `{% include %}`, and the exact function signature a level-3 prompt returns.
- What this is built on — the library that renders level 2 templates.
- [How to call a model](../engine/llm-node.md) — the node kind that calls the prompt you write.
- How to check a project before committing — what running check actually validates,
  prompts included.
- What happens when a model is called — the guarantees
  around the call itself, once the prompt has rendered.
- Five cases of dynamic input and output shape — what an input or output field looks
