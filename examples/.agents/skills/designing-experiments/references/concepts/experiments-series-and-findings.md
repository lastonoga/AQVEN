...
A run proves one case. An experiment writes a question down before any data, a series answers it on many cases, and a finding records the answer in the project.

## Contents

- [In short](#in-short)
- [The pieces](#the-pieces)
- [One factor per experiment](#one-factor-per-experiment)
- [Working and held-out cases](#working-and-held-out-cases)
- [Verdicts and signals](#verdicts-and-signals)
- [Findings and `FINDINGS.md`](#findings-and-findingsmd)
...
One run proves one case. `aqven check` proves the wiring. Neither says whether a flow works
...
[How to write an experiment](../engine/experiments.md) covers every key.
...
into the metric. [How a series decides](how-a-series-decides.md) explains the intervals behind all
five verdicts.
...
Both files are generated, and a finding is written once. `aqven check` reports an edited finding
...
- [How to write an experiment](../engine/experiments.md)
- How to run a series
- How to read a series
- How an agent takes a task to a reliable flow: the whole loop, round by round.

## See also

- [How a series decides](how-a-series-decides.md): intervals, margins, repeats and judges in plain
  words.
- The engineering loop: where experiments sit between finding a cause and
  shipping a fix.
- [Experiments reference](../reference/experiments.md) and Findings reference: every
  key of both files.
