...
from support import SAMPLES_DIR, case_flow, resume_request
...
pytestmark = [pytest.mark.aqven_engine, pytest.mark.anyio, pytest.mark.usefixtures("aqven_engine")]
...
SPLIT_BALLOTS: Final = tuple(
    node_output(
        "vote__ballot",
        {"rationale": SPLIT_RATIONALE, "intent": intent, "confidence": SPLIT_CONFIDENCE},
        item_index=index,
    )
...
@pytest.fixture
def scripted_painter(aqven_engine: EngineSession) -> None:
