...
`aqven.*`, который одновременно уезжает в Langfuse (человек смотрит глазами) и в наш Postgres
...
export const observability = new VoltAgentObservability({
  serviceName: "aqven-runtime",
  serviceVersion: gitSha,
  resourceAttributes: { "aqven.deployment": env.deployment },
  spanFilters: { enabled: true, instrumentationScopeNames: ["aqven", "voltagent", "ai"] },
...
export const exportLangfuseSpan: ShouldExportSpan = ({ otelSpan }) =>
  isDefaultExportSpan({ otelSpan }) || otelSpan.attributes["aqven.schema_version"] !== undefined;
```
...
|---|---|---|
| `OtelTraceSink` | `@aqven/obs-otel` | единственное место, где живут `@opentelemetry/api` и `@langfuse/tracing` |
| `NullTraceSink` | `@aqven/obs-otel` | тесты и `dry_run`, нулевая стоимость |
| `RecordingTraceSink` | `@aqven/obs-otel` | снимает спаны в память для unit-тестов атрибутов |

`withRunScope` внутри `OtelTraceSink` — это `propagateAttributes` из `@langfuse/tracing`:
`traceName`, `userId`, `sessionId = aqven.run_id`, `tags`, `version`, `environment`. Ограничение
...
flowchart TB
  ...
  subgraph adapter["@aqven/obs-otel"]
  ...
  NX --> AF
  AF -->|"Attributes: aqven.* + gen_ai.* + langfuse.*"| TS
  TS --> SP
...
|---|---|---|---|
| `aqven.schema_version` | int | `1` | версия этой схемы атрибутов |
| `aqven.tenant_id` | string | `01930b4c-...` | арендатор, ключ RLS |
| `aqven.workflow_id` | string | `invoice_extract` | стабильный id воркфлоу |
| `aqven.workflow_version` | int | `12` | версия спеки, давшая результат (§8.11 №77) |
| `aqven.ir_hash` | string | `sha256-9f2c...` | хэш канонизированного IR (RFC 8785) |
| `aqven.run_id` | string | `01930b4c-...` | == `session.id` в Langfuse |
| `aqven.run_mode` | string | `live` | `live` \| `replay` \| `eval` \| `dry_run` |
| `aqven.node_id` | string | `extract_totals` | стабильный id узла в графе |
| `aqven.node_type` | string | `llm` | `llm` \| `tool` \| `router` \| `map` \| `gate` \| `subflow` \| `human` |
| `aqven.node_attempt` | int | `2` | номер попытки, 1-based |
| `aqven.parent_node_id` | string | `stage_parse` | родитель в доменном графе, не в дереве спанов |
| `aqven.stage_id` | string | `parse` | стадия для матрицы стадий (§7) |
| `aqven.loop_iteration` | int | `3` | итерация цикла, `-1` вне цикла |
| `aqven.checkpoint_id` | string | `01930b4c-...` | строка в `app.run_nodes` |
| `aqven.fork.parent_run_id` | string | `01930b4c-...` | из какого прогона форкнуто |
| `aqven.fork.from_node_id` | string | `verify` | с какого узла форкнуто |

`aqven.parent_node_id` и `aqven.stage_id` нужны потому, что дерево OTel-спанов — это дерево вызовов,
...
| Ключ | Тип | Пример | Смысл |
|---|---|---|---|
| `aqven.prompt.template_id` | string | `extract_totals@v7` | шаблон из реестра |
| `aqven.prompt.template_version` | int | `7` | |
| `aqven.prompt.template_sha256` | string | `sha256-1a0f...` | хэш **шаблона** |
| `aqven.prompt.rendered_sha256` | string | `sha256-b731...` | хэш **отрендеренного текста** |
| `aqven.prompt.system_sha256` | string | `sha256-44de...` | system-часть отдельно, дедуплицируется |
| `aqven.prompt.rendered_preview` | string | `"Ты извлекаешь…[+18422 bytes]"` | ≤ 4096 символов |
| `aqven.prompt.rendered_size` | int | `22518` | байт |
| `aqven.prompt.rendered_truncated` | boolean | `true` | превью не полное |
| `aqven.prompt.rendered_ref` | string | `blob://sha256/b731...` | полное тело |
| `aqven.prompt.variables_json` | string | `{"invoice.total":"1 240,00 EUR"}` | мапа слот → превью значения |
| `aqven.prompt.messages_count` | int | `4` | |
| `aqven.prompt.cache_breakpoints` | int | `1` | число точек кэширования, см. [08. Промты](08-prompts.md) |
| `aqven.prompt.estimated_tokens` | int | `5312` | gpt-tokenizer × коэффициент профиля |
...
| `aqven.slot.sources` | string[] | `["node:parse_pdf","input"]` |
| `aqven.slot.origin_span_ids` | string[] | `["4bf92f3577b34da6",""]` |
| `aqven.slot.confidences` | number[] | `[0.91,-1]` |
| `aqven.slot.value_sha256` | string[] | `["sha256-0c1d...","sha256-77ab..."]` |
| `aqven.slot.required` | boolean[] | `[true,true]` |
| `aqven.slot.trust` | string[] | `["trusted","untrusted"]` |
| `aqven.slot.missing` | string[] | `["customer.vat_id"]` |
| `aqven.slot.count` | int | `2` |
...
`aqven.slot.trust` питает правило §8 №19 (инъекции) и политики видимости.

Массивы выбраны вместо одной JSON-строки потому, что бэкенды трасс умеют фильтровать по массивам
атрибутов, а по JSON-строке — нет. При `aqven.slot.count > 64` фабрика переключается на
`aqven.slot.provenance_json` + блоб, чтобы не упереться в `attributeCountLimit = 128`.
...
| Ключ | Тип | Пример |
|---|---|---|
| `aqven.config.hash` | string | `sha256-5e90...` |
| `aqven.config.sources` | string[] | `["node_default","workflow_override","run_override","env"]` |
| `aqven.config.preview` | string | `{"temperature":0.9,"tools":[]}` |
| `aqven.config.ref` | string | `blob://sha256/5e90...` |
| `aqven.config.model` | string | `anthropic/claude-opus-4` |
| `aqven.config.model_alias` | string | `reasoning_default` |
| `aqven.config.model_profile` | string | `anthropic_strict_v3` |
| `aqven.config.temperature` | number | `0.9` |
| `aqven.config.max_tokens` | int | `4096` |
| `aqven.config.seed` | int | `7` |
| `aqven.config.timeout_ms` | int | `120000` |
| `aqven.config.retry_policy` | string | `exp_backoff:3` |
| `aqven.config.cache_policy` | string | `read_write` |

`aqven.config.hash` — ключ ответа на «почему два прогона разошлись»: равные хэши означают, что дело
в данных или в недетерминизме модели, а не в настройках. `aqven.config.sources` сохраняет порядок
...
| Ключ | Тип | Пример |
|---|---|---|
| `aqven.rules.evaluated_count` | int | `14` |
| `aqven.rules.fired` | string[] | `["R-33","R-C4"]` |
| `aqven.rules.fired_actions` | string[] | `["retry","redact"]` |
| `aqven.rules.fired_severities` | string[] | `["warn","info"]` |
| `aqven.rules.blocked` | boolean | `false` |
| `aqven.rules.details_json` | string | превью деталей |
| `aqven.guard.schema_valid` | boolean | `true` |
| `aqven.guard.schema_errors` | string | превью ошибок zod |
| `aqven.guard.repair_attempts` | int | `1` |
| `aqven.guard.outcome` | string | `ok` \| `refusal` \| `truncated` |
| `aqven.guard.degraded` | boolean | `true` при любом фолбэке (§8 №52) |

`aqven.guard.outcome` — три исхода, а не два: `truncated` нельзя чинить ремонтом JSON (DECISIONS,
...
|---|---|---|---|---|---|
| `aqven.budget.scope` | string | `run` | `aqven.cost.input_usd` | number | `0.0036` |
| `aqven.budget.limit_usd` | number | `2.50` | `aqven.cost.output_usd` | number | `0.0051` |
| `aqven.budget.spent_usd` | number | `1.83` | `aqven.cost.cache_read_usd` | number | `0.0002` |
| `aqven.budget.remaining_usd` | number | `0.67` | `aqven.cost.cache_write_usd` | number | `0.0009` |
| `aqven.budget.limit_tokens` | int | `2000000` | `aqven.cost.total_usd` | number | `0.0098` |
| `aqven.budget.spent_tokens` | int | `1412903` | `aqven.cost.pricing_version` | string | `2026-09-01` |
| `aqven.budget.exceeded` | boolean | `false` | `aqven.cost.estimated` | boolean | `false` |
| `aqven.budget.action` | string | `none` | `aqven.tokens.input` | int | `1200` |
| `aqven.timing.queue_ms` | int | `41` | `aqven.tokens.output` | int | `340` |
| `aqven.timing.render_ms` | int | `7` | `aqven.tokens.cache_read` | int | `900` |
| `aqven.timing.provider_ms` | int | `3182` | `aqven.tokens.cache_write` | int | `0` |
| `aqven.timing.ttft_ms` | int | `612` | `aqven.tokens.reasoning` | int | `1104` |
| `aqven.timing.validate_ms` | int | `12` | `aqven.result.status` | string | `ok` |
| `aqven.cache.hit` | boolean | `false` | `aqven.result.error_kind` | string | `schema` |
| `aqven.cassette.id` | string | `cas_01930b...` | `aqven.cassette.match` | string | `exact` |

Домен `aqven.budget.action`: `none` \| `warn` \| `downgrade_model` \| `abort`.
Домен `aqven.result.status`: `ok` \| `retried` \| `failed` \| `skipped` \| `blocked` \| `cached`.
Домен `aqven.result.error_kind`: `provider` \| `timeout` \| `schema` \| `guard` \| `budget` \| `internal`.

`aqven.cost.pricing_version` обязателен: стоимость должна быть пересчитываемой задним числом при
исправлении прайс-листа. Стандартного OTel-атрибута для стоимости не существует — это наша зона.

### 2.7 Правило версии схемы

```
aqven.schema_version = 1
```

- Добавление ключа — **не** bump. Читатели обязаны игнорировать незнакомые `aqven.*`.
...
1. Наши гейты, UI, дифф прогонов, бюджет и экспорт читают **только** `aqven.*`.
...
| Сырой ответ модели | 4096 симв. | blob | при `aqven.guard.outcome = truncated` тело сохраняется как есть, без ремонта |
...
| Эффективная конфигурация | Postgres `app.runs.effective_config` | `aqven.config.hash` + превью | воспроизводимость, §8 №86 |
...
| Версии воркфлоу, граф, определения узлов | Postgres | ссылкой `aqven.workflow_version`, `aqven.ir_hash` | наш домен |
...
Обратный путь: в `langfuse.observation.metadata` кладём `aqven.run_id` и `aqven.node_id`, чтобы из UI
...
| Группировка прогона | `propagateAttributes({ sessionId: runId, userId, tags, version })` | `aqven.run_id` == `session.id` |
| Типы observation | `asType: "generation" \| "tool" \| "agent" \| "retriever" \| "guardrail" \| "evaluator" \| "chain"` | бесплатная семантика для UI: наш `aqven.node_type` мапится сюда таблицей |
...
| TTFT | `completionStartTime: Date` | дублирует `aqven.timing.ttft_ms` |
    ...
    "rules": {
      "nursery": {
        "noRestrictedImports": {
          ...
          "options": {
            "paths": {
              "@langfuse/tracing": "Только @aqven/obs-otel",
              "@langfuse/otel": "Только @aqven/obs-otel",
              "@langfuse/client": "Только @aqven/obs-langfuse",
              "@opentelemetry/api": "Только @aqven/obs-otel"
            }
          }
        }
      }
...
| `aqven.*` атрибуты и наш Postgres от бэкенда не зависят | гейты, кассеты, провенанс и экспорт продолжают работать при полностью отключённом Langfuse |
...
| 2 | Агрегированный вид: повторы свёрнуты, циклы нарисованы циклами | `aqven.node_id`, `aqven.loop_iteration`, `aqven.node_attempt` | группировка спанов по `aqven.node_id`: агрегированный режим схлопывает все `loop_iteration` и `node_attempt` в один узел со счётчиком и суммой стоимости; развёрнутый режим даёт узел на вызов |
| 3 | Группировка по стадиям | `aqven.stage_id` | ELK-группы (`elk.hierarchyHandling: INCLUDE_CHILDREN`); родитель обязан идти в массиве `nodes` раньше детей |
| 4 | Инспектор узла: вход с провенансом, клик ведёт к источнику | `app.run_nodes.provenance` + `aqven.slot.*` | каждая строка слота несёт `origin_span_id`; клик = переход на узел, у которого `span_id` равен этому значению; для `source = input` переход на вход прогона |
| 5 | Отрисованный промт с подсветкой слотов | `aqven.prompt.rendered_ref` (тело из blob) + карта диапазонов подстановки | рендерер промтов возвращает `SlotRange[] = {slot, offset, length}` вместе с текстом; это контракт [08. Промты](08-prompts.md), восстанавливать подсветку поиском подстроки запрещено — значение может встретиться несколько раз |
| 6 | Сырой ответ и распарсенный выход | blob по `aqven.response.raw_ref` и чекпоинт `app.run_nodes.output` | два таба; при `aqven.guard.outcome = truncated` таб «распарсенный» показывает отказ разбора, а не починенный JSON |
| 7 | Результаты проверок, ретраи, фолбэки | `app.run_nodes.rule_firings`, `aqven.rules.*`, `aqven.guard.*`, `aqven.node_attempt` | список попыток узла с исходом каждой; `aqven.guard.degraded = true` красит попытку как деградацию (§8 №52) |
| 8 | Стоимость и латентность узла | `aqven.cost.*`, `aqven.tokens.*`, `aqven.timing.*` | значения считаются нами по `aqven.cost.pricing_version`, а не берутся из Langfuse |
...
| 11 | Fork с узла k | `timeTravel` + **наша** таблица `app.run_lineage` | VoltAgent не сохраняет `replayedFromExecutionId` / `replayFromStepId` в типизированные поля ни в `@voltagent/postgres@2.1.3`, ни в libsql — линию форков ведём сами; в спанах это `aqven.fork.parent_run_id` и `aqven.fork.from_node_id` |
| 12 | Дифф двух прогонов или версий по узлам | `app.run_nodes` обоих прогонов | соединение по `aqven.node_id`; на каждый узел сравниваются пять хэшей и метрики: `config.hash`, `prompt.template_sha256`, `prompt.rendered_sha256`, `output_sha256`, `aqven.result.status`, дельты `cost` и `latency`. Первый несовпавший хэш в этом порядке и есть причина расхождения — диффа текстов не требуется |
| 13 | Точки останова перед узлом или при провале проверки | `suspend()` + `suspendSchema` / `resumeSchema` на шаге | обёртка шага читает список брейкпоинтов из `workflowState` и зовёт `suspend({reason:"breakpoint", nodeId})` до вызова модели либо после `aqven.guard.schema_valid = false`; продолжение — штатный `resume` с типизированным `resumeData`, возможно с правленым входом |
| 14 | Панель «почему» | `app.run_nodes.rule_firings`, `aqven.rules.fired[]` / `fired_actions[]` / `fired_severities[]`, `aqven.config.sources[]`, `aqven.budget.action` | панель отвечает тремя блоками: какое правило сработало и с каким действием; откуда взялось каждое поле эффективной конфигурации (порядок мержа из `config.sources`); что сделал бюджет |
| 15 | Водопад стоимости и латентности | `app.run_nodes` (`started_at`, `ended_at`, `aqven.cost.total_usd`) | не чарт, а виртуализированная таблица: строка = спан, отступ = глубина, позиция полосы = проценты от окна прогона; `d3-scale@4.0.2` подключаем только если понадобятся оси или log-шкала |
...
| 18 | Матрица стадий: элементы датасета × стадии | `aqven.stage_id` + scores прогонов датасета | ячейка = агрегат исходов проверок стадии по элементу; источник scores — Langfuse, агрегаты, влияющие на гейт, зеркалятся в Postgres (§4) |
| 19 | Lineage значения по стадиям и промтам | `app.run_nodes.provenance` | рекурсивный обход `origin_span_id` вверх до `source = input`, вниз — поиск всех слотов, чей `value_sha256` совпал; «в каких промтах использовалось» = список узлов, где значение встретилось в `aqven.prompt.variables_json` |
| 20 | Таймлайн цикла | `aqven.loop_iteration` | тот же компонент, что и водопад, с другим группирующим ключом: swimlane, строка = итерация |
...
внутри `@aqven/llm` — единственного пакета, которому разрешено импортировать `ai` и `@ai-sdk/*`.
...
Режимы по `aqven.run_mode`:

| Режим | `CassetteMissPolicy` | Поведение |
|---|---|---|
| `live` | `record` | вызываем провайдера, пишем кассету, `aqven.cache.hit = false` |
| `replay` | `fail` | промах кассеты — ошибка `CassetteMiss` с `requestHash`, молчаливый вызов провайдера запрещён |
| `eval` | `passthrough` | кассеты читаются, промах идёт к провайдеру, факт отмечается `aqven.cassette.match = miss` |
| `dry_run` | `fail` | без сети вообще |

`aqven.cassette.match` принимает `exact` \| `miss`; значение `fuzzy` зарезервировано, но в v1
...
| Отрисованный промт (`aqven.prompt.rendered_sha256`) | побитово | тот же шаблон и те же значения слотов |
| Эффективная конфигурация (`aqven.config.hash`) | побитово | канонизация RFC 8785 |
...
| Что | Почему |
|---|---|
| `aqven.timing.*`, `ttft`, латентность, водопад | физическое время не воспроизводится и не должно |
| Идентификаторы: `trace_id`, `span_id`, `execution_id`, `run_id` | новый прогон — новые идентификаторы; связь через `aqven.fork.parent_run_id` |
...
| Стоимость при пересчёте старым прайсом | восстанавливается по `aqven.cost.pricing_version`, но новый прогон посчитает текущим прайсом |
...
5. Результат пишется как новый прогон с `aqven.fork.parent_run_id` и `aqven.fork.from_node_id`,
...
считанную по `aqven.prompt.estimated_tokens` и текущему прайсу.
...
`GATE_UNAVAILABLE`; `warn` — деградация (`aqven.guard.degraded`), ретрай, срабатывание бюджета;
...
| №76 сдвиг распределения в проде | дрейф входов и исходов | доли `aqven.result.status`, `aqven.guard.outcome`, `aqven.rules.fired` по версии воркфлоу, неделя к неделе; плюс семплирование прода в очередь разметки Langfuse | значимое расхождение → элементы уходят в `annotationQueues`, пополняя датасет |
...
| `guard_degraded_total{node_id}` | `aqven.guard.degraded` | каждая деградация — скрытая потеря качества |
...
| Бюджет арендатора | `budget_exceeded_total` > 0 | warning | уведомление владельца, `aqven.budget.action` уже применён автоматически |
...
| Дрейф прода | сдвиг долей `aqven.result.status` неделя к неделе значим | info | пополнение очереди разметки |
...
4. **Провенанс слотов: параллельные массивы против `aqven.slot.provenance_json`.** Массивы выбраны
   ...
   на JSON-строку и bump `aqven.schema_version`.
...
13. **Сборка канваса живого графа.** Не проверена связка Vite 8.3.0 ([ADR-0024](adr/0024-studio-on-vite.md)) с
