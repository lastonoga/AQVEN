...
- **`zIndex`.** По умолчанию рёбра рисуются **под узлами**; ребро, привязанное к узлу с `parentId`,
  — наоборот, **над** узлами. Отсюда каша слоёв. Лечится `defaultEdgeOptions={{ zIndex: 1 }}` плюс
  явный `zIndex` по ролям: `0` для `wfgroup`, `1` для `wf`, `2` для рёбер. Баг #4831: при выборе
  группы дети прыгают с `zIndex` 10 на 1000, а рёбра и `EdgeLabelRenderer` не обновляются —
  не полагаться на автоэскалацию.
