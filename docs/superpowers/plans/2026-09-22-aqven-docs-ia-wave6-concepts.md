...
- [x] Find the current `"Concepts"` group:
  ```js
  {
    label: "Concepts",
    items: [
  ...
  ```
- [x] Replace with (order matches design doc §4's Концепции list — items 1 and 9 stay where they are,
...
- [x] Run `pnpm --filter @aqven/site check` (missing-file errors for the 9 new slugs are expected and
...
- [x] Frontmatter: `title: Files as source of truth`, one-sentence `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §3 in full. **Scope: layout + identity only —
  do NOT cover editing mechanics** (direct edit vs `flow_patch`/CAS/hooks/drafts) — that's Task 2.6
  (item 11), a separate page.
- [x] Cover: a node is a *folder* (`flows/<flow_id>/nodes/<node_id>/`), not a single file — correct the
  ...
  disk.
- [x] Re-verify live before writing: confirm `examples/lumen/flows/judge_panel/`'s file tree still
  matches the research section's captured `find` output, and that `examples/lumen/aqven.yaml` still has
  a `renames:` entry.
- [x] Real example: the `examples/lumen/flows/judge_panel/` file tree from the research section (already
  verified there) — a small, complete flow showing directory-per-node layout and flat nested placement.
- [x] Cross-references (from the research section, quote directly — link to, do not repeat): "link to
...
- [x] Run `pnpm --filter @aqven/site check`, commit:
...
- [x] Frontmatter: `title: Agent, Inference, and the llm node`, one-sentence `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §4 in full. This page is, per the design doc's
...
- [x] Cover: the three entities (Agent = "how to call," Inference = "what to ask," Node = "when/with
...
- [x] Re-verify live before writing: confirm `examples/lumen/flows/support_case/nodes/drafts/*.node.yaml`
...
- [x] Real example: the `drafts/*` reuse example from the research section (three Node files, same
...
- [x] Cross-references (quote directly): "Must link to (not repeat): `apps/site/src/content/docs/engine/
...
- [x] Frontmatter: `title: Ten kinds of nodes`, one-sentence `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §5. This is a pure map/index page — minimal
  new content, no new example needed per the research section.
- [x] Re-verify live before writing: confirm the Node discriminated union in
  ...
  section.
- [x] List all 10 with a one-line purpose each, reusing each kind's established framing (quoted in full
...
- [x] Optional grouping (per the research section, a judgment call, not required): three kinds
...
- [x] Cross-references: link out to all 10 `/engine/<kind>-node/` pages; link to
...
- [x] Frontmatter: `title: A run survives a process crash`, one-sentence `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §2 in full.
- [x] Cover: the whole flow run is one durable workflow; checkpoint granularity is NOT uniformly "one
...
- [x] Re-verify live before writing: confirm `packages/aqven/src/aqven/spec/nodes.py`'s
...
- [x] Real example: the run-status query cycle from the research section (start a run, kill the process,
...
- [x] Cross-references: "link to `/concepts/what-this-is-built-on/` (DBOS row) instead of repeating its
...
- [x] Frontmatter: `title: Three prompt levels`, one-sentence `description`.
- [x] **Read `apps/site/src/content/docs/engine/prompts.md` first.** Ground in
...
- [x] Cover the capability/guarantee ladder from the research section: level 1 is enough with no
...
- [x] **Do not mention GEPA / prompt optimization** as a level-3 tradeoff — `optimize` is an
...
- [x] Re-verify live before writing: skim `packages/aqven/src/aqven/check/conditions.py` and
...
- [x] Real example: the level 1→2 escalation via optional attachments in `triage.prompt.md`; the level-2
...
- [x] Cross-references: "Must link to (not repeat): `apps/site/src/content/docs/engine/prompts.md` for
...
- [x] Frontmatter: `title: Five cases of dynamic input and output shape`, one-sentence `description`.
- [x] **Read `apps/site/src/content/docs/engine/dynamic-shape.md` first.** Ground in
...
- [x] Cover the 5-case escalation ladder from the research section (allowed-set → discriminated
  ...
  a static guarantee.
- [x] Cover why this matters: what's lost by collapsing into "schema from data" (static type-checking
...
- [x] Re-verify live before writing: confirm `examples/lumen/flows/support_case/nodes/triage/
...
- [x] Real example: the `triage`/`intake_extra` case-4 example from the research section (a genuinely
...
- [x] Cross-references: "link to `apps/site/src/content/docs/engine/dynamic-shape.md` (case 5
...
- [x] Frontmatter: `title: What happens when a model is called`, one-sentence `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §8 in full.
- [x] Cover the three real outcomes (`ok`/`refusal`/`truncated`), decided before parsing from the
  provider's finish reason — so a cut-off answer can never be silently "repaired" into a fake complete
  object.
- [x] Cover the two genuinely automatic retry mechanisms and their layers: transport-level retry
...
- [x] Cover PII redaction precisely scoped, per the research section: the always-on, zero-config
  ...
  runs for every call.
- [x] Before writing, check whether `apps/site/src/content/docs/engine/llm-node.md` already documents
...
- [x] Re-verify live before writing: confirm `packages/aqven/src/aqven/runtime/vocabulary.py`'s
  ...
  found.
- [x] Real example: the real transport-retry test scenario (a 429 with `retry-after: 2`, retried,
...
- [x] Cross-references: "link to `/concepts/what-this-is-built-on/` instead of re-pitching 'this is a
...
- [x] Frontmatter: `title: How to find the node where a workflow went wrong`, one-sentence
  `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §10 in full (including its "Verification
  note").
- [x] Cover the 4-field execution address (`node_id`, `branch_key`, `iteration`, `item_index`) and why
...
- [x] Re-verify live before writing: confirm `examples/lumen/flows/support_case/nodes/drafts/`,
  ...
  `vote__ballot`).
- [x] Real example: reuse the already-established showcase node ids from `/engine/parallel-node/`,
...
- [x] Cross-references: "link to `/concepts/engineering-loop/` (already tells the reader why addressing
...
- [x] Frontmatter: `title: Two ways to change a project`, one-sentence `description`.
- [x] Ground in `docs/research/site-ia-concepts-wave6.md` §11 in full.
- [x] Cover both paths: direct file edit (validated AFTER the write, by a project hook running the real
...
- [x] Cover why this matters especially for an agent: it can fire off many fast edits with no human
...
- [x] **Do not use "flow_check" as if it were a real function/tool name** — per the research section,
...
- [x] **Do not overstate what `flow_patch` covers** — per the research section, the currently
...
- [x] Re-verify live before writing: confirm the real project hook still exists at
...
- [x] Real example: a minimal `flow_patch` request/response pair (a `set` operation with an expected
...
- [x] Cross-references: "must link to (not restate) `/mcp-cli/edit-a-flow/` (every operation, the full
...
- [x] Run `pnpm --filter @aqven/site check`, then `pnpm --filter @aqven/site build`. All 11 Concepts
  pages plus all previously published pages must build with 0 errors.
- [x] Run `node apps/site/scripts/generate_llms.mjs --check`.
- [x] Spot-check the sidebar renders all 11 Concepts entries in the designed order (item 1, 2, 3, 4, 5,
...
**Status: Wave 6 complete, merged into `codex/codex-cli-studio-chat` (commit `838b39f`). This was the
last content wave — all 6 areas plus all 11 Concepts pages are now published.**

- All 9 pages plus sidebar wiring implemented via subagent-driven-development, one implementer subagent
  per task, sequentially. No plan corrections were needed mid-execution — the upfront research pass
  (`docs/research/site-ia-concepts-wave6.md`) and its two flagged findings (the refusal/truncated gap on
  Task 2.4, the examples/lumen correction on Task 2.5) held up against live re-verification in every
  task; no implementer found new drift beyond the benign reference venv-path artifact (regenerated and
  committed twice: `ba8eb8f` before merge, `838b39f` after).
- Task 2.6's implementer made one small follow-up commit (`a5602a6`) to tighten wording that slightly
  understated the real project hook — not a factual correction, a clarity fix.
- Full build: 81 pages (up from 72 after Wave 5), `pnpm --filter @aqven/site check` clean (0
  errors/warnings/hints beyond pre-existing unrelated landing-page hints), `llms.txt --check` clean both
  before and after merge.

## Remaining work on the design doc (not part of any wave plan)

The only work left against `docs/superpowers/specs/2026-09-21-aqven-docs-ia-design.md` is the §7
cross-wave example-simplification pass — go page-by-page across the entire published site (all 6 areas +
11 Concepts pages) and simplify every how-to/concept page's example to be small and self-contained,
decoupling from the full showcase project where the design doc's §7 follow-up applies (Start's tutorial
pages are exempt — the design doc calls out the single sequential showcase walkthrough there as
deliberate). Also covers the directory-structure-for-nested-nodes follow-up recorded in Wave 2's plan
file. This needs its own pass, not a new content wave.
