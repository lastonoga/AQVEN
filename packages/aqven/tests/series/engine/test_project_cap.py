...
from aqven.series.model import LaunchPlan
...
def planned(root: Path, settings: MemorySettings) -> LaunchPlan:
    services = build_series_services(root, ProjectWorkspace(root), settings, StubAnalyst(), RecordingFindings(), "test")
    return asyncio.run(SeriesService(services).launch_plan(ExperimentId("triage_agents"), LaunchRequest()))
...
def test_the_series_reads_its_cap_from_aqven_yaml(tmp_path: Path) -> None:
    ...
    launch = planned(root, MemorySettings())

    assert (launch.project_cap_usd, launch.project_cap_source) == (Decimal(TINY_PROJECT_CAP), "project")
    assert (launch.cap_usd, launch.needs_approval) == (Decimal(TINY_PROJECT_CAP), False)
...
def test_a_local_override_beats_aqven_yaml(tmp_path: Path) -> None:
    ...
    launch = planned(root, settings)

    assert (launch.project_cap_usd, launch.project_cap_source) == (Decimal(LOCAL_OVERRIDE), "override")
    assert (launch.cap_usd, launch.needs_approval) == (Decimal(LOCAL_OVERRIDE), False)


def test_a_project_without_a_research_block_keeps_one_dollar(tmp_path: Path) -> None:
    launch = planned(write_project(tmp_path), MemorySettings())

    assert (launch.project_cap_usd, launch.project_cap_source) == (Decimal("1.00"), "default")
