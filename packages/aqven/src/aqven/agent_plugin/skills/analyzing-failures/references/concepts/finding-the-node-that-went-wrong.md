...
the `run_*` tools take this same 4-field address as an argument: `run_get_node` reads
...
- How to run a step over a collection — what a `map` node is and how its items are
  defined.
- How to start and follow runs as an agent — `run_get_node` and `run_fork`, which take
