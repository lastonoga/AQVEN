...
- [Steps](#steps)
  - [`dev` to search, `holdout` to decide](#dev-to-search-holdout-to-decide)
  - [A look without an experiment](#a-look-without-an-experiment)
  - [Example](#example)
- [See also](#see-also)
...
- **Write the experiment first, then run `aqven_check`.** There is no tool that creates an experiment:
  it is a file, and you write it like any other ([How to write an experiment](../engine/experiments.md)).
...
In the showcase project, `reply_overpromise_risk` asks whether the `polish` range
...
line. How to run a series lists every flag and exit code.

## See also

- How an agent takes a task to a reliable flow — the rounds these tools serve.
- How to start and follow runs as an agent — every attempt of a series is one of
  these runs, and `run_get_node` opens any of them.
- How to use Research in Studio — the same experiments and series, read and
  approved by a person.
- How to read a series — what to do after each verdict and reason.
- How to write a custom evaluator — checks an experiment scores on every
  attempt.
- Experiments reference and Findings reference — every
  key of `experiment.yaml` and of a finding file.
- Experiments, series and findings — why the question is
