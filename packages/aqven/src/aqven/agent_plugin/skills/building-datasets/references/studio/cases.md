# How to work with cases in Studio

Browse a flow's saved cases, filter them by tag, add cases by CSV or through the chat, run one case, or select several and run a look over the stages you pick.
...
- **Add cases with the chat or a CSV.** **Ask the agent to add cases** (or **Ask the agent to write
  ...
  `aqven check`. **Upload CSV** opens a dialog with a guide to the columns this flow expects and
...
- **Attach a media file to a case.** Studio writes the file into `datasets/<dataset_id>/`, next to the
  ...
  [how to keep case media as files in the project](../engine/dataset-media-files.md). Media from a CSV import
  and cases drafted from a run point at blobs. Run `aqven datasets materialize` to turn them
  into files.
- **Lock a fix with a case.** From a run that went wrong, **To cases** drafts a dataset case from that
  run and hands it to the chat. See How to investigate a run.
...
- How to use Research in Studio: the experiments that select these cases, and the
  series that answer them.
- How to follow and read a series in Studio: the page a look opens on.
- [How to write an experiment](../engine/experiments.md): selecting cases with `cases.tags`.
- [How to keep case media as files in the project](../engine/dataset-media-files.md): the two path forms,
  private media and moving old blob-backed cases to files.
- [Datasets reference](../reference/datasets.md): every key of a case.
- How to investigate a run: the run that **Start run** opens, and the
  **To cases** action.
