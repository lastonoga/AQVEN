...
- **`llm`** — calls a model through a typed inference. [How to call a model](../engine/llm-node.md); for
...
- **`map`** — runs one body node once per item in a collection.
  [How to run a step over a collection](../engine/map-node.md).
...
- **`narrow`** — narrows a dynamic value to a type, turning an open-ended value into ordinary typed
  fields. How to narrow a dynamic value to a type.
...
- Agent, Inference, and the llm node — what's actually
  behind one `llm` node.
