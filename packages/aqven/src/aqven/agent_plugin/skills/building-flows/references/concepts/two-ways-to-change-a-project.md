...
A direct file edit is checked after the write by a project hook; flow_patch is checked before the write by a compare-and-swap that can reject it outright.

## Contents

- [In short](#in-short)
- [Direct edit: checked after the fact, if at all](#direct-edit-checked-after-the-fact-if-at-all)
- [`flow_patch`: checked before the write, or not written at all](#flow_patch-checked-before-the-write-or-not-written-at-all)
- [A rename through `flow_patch` leaves a trail; a hand-edit doesn't](#a-rename-through-flow_patch-leaves-a-trail-a-hand-edit-doesnt)
- [Why this split matters more for an agent](#why-this-split-matters-more-for-an-agent)
  - [Example](#example)
...
What catches this is `aqven check`, described in full on
how to check a project before committing. Run by hand, it's a normal CLI command.
...
`aqven check` is also what CI runs before a build can ship.
...
[how to edit a flow's structure as an agent](../mcp-cli/edit-a-flow.md) — this page only establishes the
...
Files as source of truth covers the `renames:` list that lives
...
[how to edit a flow's structure as an agent](../mcp-cli/edit-a-flow.md).
...
field on one node — and run `aqven check` yourself right after, whether or not a hook does
...
- How to check a project before committing — what `aqven check` actually
  validates.
- How to read a project's structure as an agent — reading files
  directly before you decide which of these two paths to take.
- Files as source of truth — what a project looks like on disk,
