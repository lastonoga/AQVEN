...
Add a call node that runs another flow as a single step, passing it typed input and getting back whatever that flow declares as its own output.
...
- A flow can't call itself, directly or through a chain of `call` nodes — `aqven check`
...
- [How to call a model](llm-node.md) — `tie_break`, the OpenAI judge `judge_panel`'s `decide`
  step calls in only when the panel splits.
- [How to branch into parallel steps](parallel-node.md) — `judges`, which runs three judge
  models at once inside `judge_panel`.
- [How to write a step in Python](code-node.md) — `aggregate` and `pick`, `judge_panel`'s two
  `code` steps.
- How to route by a value — `decide`, which only calls `tie_break` when the
  panel's judges split.
- The engineering loop — what to do when a run's output isn't what you
  expected.
- [Node specifications](../reference/nodes.md) — every field on `CallNodeSpec`, generated from the code.
- [Flow specification](../reference/flows.md) — every field on `FlowSpec`, including `input`, `output`,
  and `returns`.
