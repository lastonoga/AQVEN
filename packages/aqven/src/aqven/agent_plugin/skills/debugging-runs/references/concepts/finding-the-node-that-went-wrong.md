...
Every step execution has a 4-field address, not just a node name, because the same node can run more than once inside one run.

## Contents

- [In short](#in-short)
- [The four fields](#the-four-fields)
- [What each construct sets](#what-each-construct-sets)
- [A container's own execution doesn't carry its children's field](#a-containers-own-execution-doesnt-carry-its-childrens-field)
- [Nesting composes](#nesting-composes)
- [Which retry isn't part of the address](#which-retry-isnt-part-of-the-address)
- [Example](#example)
...
[investigating a run](../studio/investigate-a-run.md) shows this same address under the title of whatever
...
[the `run_*` tools](../mcp-cli/runs.md) take this same 4-field address as an argument: `run_get_node` reads
...
- The engineering loop — why every execution needs its own address, and
  where finding one fits between understanding a flow and testing a fix.
- [How to investigate a run](../studio/investigate-a-run.md) — clicking through a run's matrix to one
  execution's address, input, prompt, output, and checks.
- How to branch into parallel steps — what a `parallel` node is and how its
  branches are defined.
- How to repeat a step with a limit — what a `loop` node is and how its passes
  work.
- How to run a step over a collection — what a `map` node is and how its items are
  defined.
- [How to start and follow runs as an agent](../mcp-cli/runs.md) — `run_get_node` and `run_fork`, which take
