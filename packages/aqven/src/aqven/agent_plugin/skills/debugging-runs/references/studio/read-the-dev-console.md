# How to read the dev console

What the terminal running the project server prints for runs, steps, retries, series and file changes, how to see more or less of it, where the full log is kept, and where the thread stacks of a stuck server go.
...
- [Steps](#steps)
  - [Example](#example)
- [When the server stops answering](#when-the-server-stops-answering)
- [What stays hidden at the default level](#what-stays-hidden-at-the-default-level)
- [Under the hood](#under-the-hood)
- [See also](#see-also)

## When you need this

You started Studio with `aqven dev` (or the server with `aqven serve`) and want the
...
- Read a Python warning once. Each warning prints the first time only, however many calls raise it. The
  ...
  [`W_SAMPLING_IGNORED`](../reference/diagnostics.md) warning that `aqven check` gives for the same
  agent, with the hint and the agent file below it, for example `W_SAMPLING_IGNORED: temperature is
  ignored by openrouter:openai/gpt-5-nano (reasoning model); remove it`. Run `aqven check` to
...
`aqven run` prints its run events to standard output in its own format and sends every other
line through the same console. `aqven check` uses the same filters and prints only warnings and errors next to
its report.
...
top. See What this is built on.

## See also

- How to open an existing project in Studio — starting the server whose
  console this page reads.
- [How to investigate a run](investigate-a-run.md) — the Studio page the `▶` line links to.
- How to set a secret for a provider, tool, or MCP server — the
