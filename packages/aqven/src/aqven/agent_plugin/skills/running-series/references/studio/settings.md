...
Use this right after `aqven new`, the first time you open Studio: you want working model
...
- **Other secrets** appears below Model keys only when a tool or an MCP server of the project declares
  ...
  use it.
- **Your coding agent.** `aqven new` puts a `.mcp.json` file in the folder it creates, so
...
- **Research budget** shows the project spend cap: every series runs under it and pauses for your
  ...
  again. [How to run a series](../engine/run-a-series.md) explains the cap and the override.
- **About** shows the project folder, the AQVEN version, and the update command. Run it in the project
  folder, then restart Studio.

### Example

Create the showcase project and start Studio:
...
- How to manage secrets: the CLI report of the same keys, with the same masking.
- [How to connect a model provider](../integrations/model-providers.md): declare a provider, so its key
  shows up in **Model keys**.
- How to connect AQVEN as an MCP server: the same MCP connection, with
