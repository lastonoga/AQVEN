...
Declare a provider in aqven.yaml, point an agent's model field at it, and set the one credential the provider needs.
...
- Set the credential. For most of the 28 built-in provider families that's one environment variable,
  ...
  provider catalog lists the exact name for every one of them. The one
...
- Provider catalog — every built-in provider, its model class, default
...
- How to check your model providers are configured — whether a configured
