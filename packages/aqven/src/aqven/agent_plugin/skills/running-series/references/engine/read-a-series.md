...
doesn't, fix the prompt, no experiment needed. In the showcase it does: `aqven prompt preview
...
- How a series decides: the intervals, margins and outcomes behind
  every number here.
- [How to follow and read a series in Studio](../studio/series.md): where each of these blocks sits on the
  screen.
- [How to run experiments and series as an agent](../mcp-cli/experiments-and-series.md): the same fields in
  `series_get`.
- How an agent takes a task to a reliable flow: what the next round looks like.
