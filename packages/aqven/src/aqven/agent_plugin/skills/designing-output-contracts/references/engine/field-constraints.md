# How to constrain a field's values

Every in and out field can carry length, range, pattern, and choice constraints that a model's answer must actually obey — one small example per constraint, plus what happens when one is broken.

## Contents

- [When you need this](#when-you-need-this)
- [`maxLength` — cap how long a `Text` field can be](#maxlength--cap-how-long-a-text-field-can-be)
- [`pattern` — require a `Text` field to match a regular expression](#pattern--require-a-text-field-to-match-a-regular-expression)
- [`enum` — restrict a `Text` field to a fixed list of values you write yourself](#enum--restrict-a-text-field-to-a-fixed-list-of-values-you-write-yourself)
- [`minimum` and `maximum` — bound an `Int` or `Float` field, inclusive on both ends](#minimum-and-maximum--bound-an-int-or-float-field-inclusive-on-both-ends)
- [`maxItems` — cap how many items a list field can return](#maxitems--cap-how-many-items-a-list-field-can-return)
- [There's no minimum length or minimum count](#theres-no-minimum-length-or-minimum-count)
- [A constraint only applies to the right type](#a-constraint-only-applies-to-the-right-type)
- [Every list or plain-text output field needs one](#every-list-or-plain-text-output-field-needs-one)
- [What happens when a constraint is broken](#what-happens-when-a-constraint-is-broken)
- [Optional fields and lists: `?` and `[]`](#optional-fields-and-lists--and-)
- [A list you write vs. a list the project already knows](#a-list-you-write-vs-a-list-the-project-already-knows)
- [See also](#see-also)
...
There are six of them. Each one only applies to certain types, and `aqven check` rejects a
...
length. This is why how to call a model's worked example has a constraint on every
single field in its `out:` — that's not extra rigor its author added, it's what `aqven check`
...
than written as a literal list. [Five cases of dynamic input and output shape](../concepts/five-dynamic-shape-cases.md)
...
- How to call a model — the worked example where every `out` field carries one of
  these constraints.
- [Fields and bindings](../reference/fields.md) — the generated reference for every field keyword, including
  the ones this page doesn't cover (`from`, `value`, `schema_from`).
- [Five cases of dynamic input and output shape](../concepts/five-dynamic-shape-cases.md) — allowed-sets, and
...
- [How to handle a shape you don't know in advance](dynamic-shape.md) — the `Dynamic` type, for when
  the whole output shape, not just one field's value, depends on data.
- What this is built on — the retry and guarantee layer that enforces
