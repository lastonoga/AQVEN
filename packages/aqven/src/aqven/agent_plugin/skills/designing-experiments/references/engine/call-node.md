...
- How to call a model — `tie_break`, the OpenAI judge `judge_panel`'s `decide`
  step calls in only when the panel splits.
- How to branch into parallel steps — `judges`, which runs three judge
  models at once inside `judge_panel`.
- How to write a step in Python — `aggregate` and `pick`, `judge_panel`'s two
  `code` steps.
...
- The engineering loop — what to do when a run's output isn't what you
  expected.
- Node specifications — every field on `CallNodeSpec`, generated from the code.
- Flow specification — every field on `FlowSpec`, including `input`, `output`,
  and `returns`.
