# How to find a model's real structural limits

What aqven models shapes reports for an agent's models — the nesting depth, list length and enum size they actually handle correctly, found with real, billed API calls.
...
- [Steps](#steps)
  - [Example](#example)
- [Testing a provider-specific setting](#testing-a-provider-specific-setting)
...
a real schema around nested objects, an array of objects, or a large closed set of codes, `aqven
models shapes` finds the model's actual boundary with real requests instead of a guess.

## Steps

- `aqven models shapes [TARGET] --project PATH --live` checks one target or every agent in the
  project. `TARGET` is an agent id the way `tree` lists it, or a
  `provider:model` string on its own, the same as [`models check`](check-providers.md). `--live` is
...
```bash
aqven models shapes vision --project my_project --live
```
...
- [How to check your model providers are configured](check-providers.md) — which structured output
...
- [What happens when a model is called](../concepts/what-happens-when-a-model-is-called.md) — the
  structured-output repair retry that still runs during a real flow, independent of what this command
  finds.
- How to call a model — shaping `out` fields flat and small is already the default
