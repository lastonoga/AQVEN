...
Every modality a provider takes in or produces has a ceiling, and crossing it either reshapes your data silently or refuses the call outright — a rule worth knowing for any modality, not only the ones this page happened to research.

## Contents

- [In short](#in-short)
- [The same shape, whatever the modality](#the-same-shape-whatever-the-modality)
- [Media fields carry no size limit AQVEN enforces](#media-fields-carry-no-size-limit-aqven-enforces)
- [Evidence: the input side, across three providers](#evidence-the-input-side-across-three-providers)
- [Evidence: the output side, in a harsher form](#evidence-the-output-side-in-a-harsher-form)
...
Field constraints covers `maxLength` on `Text`, `pattern` and `enum` on
...
1. **Before you wire any modality into a call, find its real current ceiling** for that specific
   ...
   `aqven models check` confirms what an agent's models support
...
- **Video generation past a duration cap (rule 3):** if a case genuinely needs a five-minute video from
  ...
  is what a `loop` node is for: `init` seeds the first segment, `body` generates
  ...
  `parallel` or `map` instead, generating every segment at once. Either way, a final
...
- **Concurrency is the same ceiling, on a different axis.** `parallel` and `map` nodes
...
- How to give an agent a tool — where a resize, trim, tile or transcode step
  ...
  lives, `wait` included
- The `code` node — deterministic logic with no blob access; confirms why media
  preparation doesn't belong here
- How to repeat a step with a limit — for chaining capped generations that depend
  on each other, one pass at a time
- The `map` node — concurrency, provider rate limits, and for chaining capped
