---
title: How to customize a node's display in Studio
description: Replace the raw JSON Studio shows for a node's input or output with a template built from sections, cards, fields, badges, lists and media.
---
...
- `{{CLI_COMMAND}} check` parses every display template and rejects one that fails to parse, is
...
This is the real `illustrate` node from the [showcase](/start/quickstart/) project. Create it
yourself with:
...
Display templates render on [python-liquid](/concepts/what-this-is-built-on/), the same engine
...
`{{CLI_COMMAND}} prompt preview` is for prompts.

## See also

- [How to write a prompt](/engine/prompts/) — the sibling file-naming and Liquid-template pattern for
  a node's prompt, on the same `.inference.yaml`.
- [Investigate a run](/studio/investigate-a-run/) — where a node's display template actually shows up:
  the input/output panels in the run inspector.
- [How to call a model](/engine/llm-node/) — the node kind `display` attaches to.
- [Inference specifications](/reference/inference/) — every field on the inference spec, including
  `display`.
