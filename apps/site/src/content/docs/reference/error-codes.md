---
title: Error codes
description: Generated index of run error codes, API error codes and check diagnostics from the AQVEN Python package.
---

<!-- Generated by tools/generate_reference.py. Do not edit this file by hand. -->

Every code below comes from the AQVEN package: the enums of failed steps and runs, the table a series uses to count an attempt, the API error codes and the diagnostics of `{{CLI_COMMAND}} check`.
...
`{{CLI_COMMAND}} check` reports these codes. A code without a template builds its message where it is found; the message names the file, the path and the fix.
