...
To regenerate this section after changing the Python package, run `python tools/generate_reference.py` in an environment where that package is installed. Run the same command with `--check` to detect stale generated pages. Do not hand-edit generated pages.
