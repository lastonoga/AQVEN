...
description: The questions an experiment has to pass before its first series — one factor, compared things as variants, ground truth and a negative control, the same population in both halves, synthetic cases that have their property, a reachable margin — and which of them the engine checks for you.
---
...
within reach. `{{CLI_COMMAND}} check` catches the structural mistakes. The rest is on you. Write the
...
| 3 | Is there exactly one factor? | `varies` names one kind (`agent`, `prompt`, `use` or `flow`) and its nodes; the compared things are variants and the measures are checks; `{{CLI_COMMAND}} check` reports no factor error |
...
[Cases that can answer the question](/concepts/case-construction/).

## What the engine checks, and what it leaves to you

`{{CLI_COMMAND}} check` refuses these before any series:
...
- Run the gate after `experiment.yaml` is written and `{{CLI_COMMAND}} check` is clean, and before the
  first series.
...
- [Experiments, series and findings](/concepts/experiments-series-and-findings/): the factor, variants and
  the experiment folder.
- [How a series decides](/concepts/how-a-series-decides/): margins, the launch plan's recommendation, and
  the noise floor.
- [Hypotheses by category](/concepts/hypothesis-categories/): where the claim comes from.
- [How to write an experiment](/engine/experiments/): every key and every error message.
