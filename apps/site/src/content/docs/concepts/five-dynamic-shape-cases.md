---
title: Five cases of dynamic input and output shape
...
2. **Which of several known-in-advance structural variants applies.** You know every shape the value
   ...
   on it with a `switch` node — see [How to route by a value](/engine/switch-node/) for the mechanism.
...
5. **The whole shape is unknown ahead of time.** Not one field of several — the entire output. The whole
   ...
   which type actually applies. [How to handle a shape you don't know in advance](/engine/dynamic-shape/)
...
in full on [How to handle a shape you don't know in advance](/engine/dynamic-shape/).
...
- Whichever case you land on, a `Dynamic` field's real schema is only exercised by `aqven check`'s
  ...
  committing](/engine/check/) for what each pass actually verifies.

## See also

- [How to handle a shape you don't know in advance](/engine/dynamic-shape/) — case 5's mechanics in
  full: declaring a field `Dynamic`, building its schema from data, and reading it back.
- [How to narrow a dynamic value to a type](/engine/narrow-node/) — turning a `Dynamic` value into an
  ordinarily typed one.
- [How to route by a value](/engine/switch-node/) — case 2's mechanism.
- [How to check a project before committing](/engine/check/) — what static checking and simulation each
  verify for a `Dynamic` field.
- [Ten kinds of nodes](/concepts/ten-kinds-of-nodes/) — `narrow` alongside the other nine node kinds.
