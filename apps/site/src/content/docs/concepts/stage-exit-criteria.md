---
title: When a stage is done
description: The exit criterion of every stage between a request and a reliable flow, the false exits that look like progress, the stage of the flow each experiment measures, and when to stop the whole loop or stop and ask.
---
...
criterion you can check, not on a feeling. The common false exits are a clean `{{CLI_COMMAND}} check`
...
| Simplest flow | one `llm` step per real decision, `code` for the rest; `{{CLI_COMMAND}} check` is clean; the prompt preview of every `llm` node is read in full; one run on a real input passed, and its output reads well in the run view | `{{CLI_COMMAND}} check` is clean |
...
| Hypotheses | each remaining failure mode has an experiment written before any number, and it passed the [validity gate](/concepts/validity-gate/) | `experiment.yaml` passes `{{CLI_COMMAND}} check` |
...
- **A clean check.** `{{CLI_COMMAND}} check` proves the wiring and a simulated run. It says nothing about
...
[research journal](/concepts/research-journal/), and check it before every held-out series. A threshold
...
- [How an agent takes a task to a reliable flow](/mcp-cli/research-loop/): the loop, round by round.
- [The engineering loop](/concepts/engineering-loop/): from an incident to a verified fix.
- [Correctness needs ground truth and a control](/concepts/metrics-and-controls/): metrics per stage.
- [The research journal](/concepts/research-journal/): where the current stage is written down.
