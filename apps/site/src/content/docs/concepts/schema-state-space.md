---
title: How big an output schema can get
description: A provider builds your output schema into a state machine before the model writes a token. Nesting, list lengths, enums and string lengths multiply its size, and past a limit the call fails. How to keep outputs small, how the failures look, and when prompted mode helps.
---
...
`{{CLI_COMMAND}} check` already refuses an output that has no bound at all. An output list without
...
| `auto` | resolved per model | `{{CLI_COMMAND}} check` reports the choice as `W_OUTPUT_MODE_RESOLVED`; pin it |

`{{CLI_COMMAND}} models check <agent> --project <package> --live` tries every mode on a small schema and
...
- Before you build around a nested or large output, run `{{CLI_COMMAND}} models shapes <agent> --project
...
- [How to find a model's real structural limits](/engine/check-shapes/): the probe and its report.
- [How to check your model providers are configured](/engine/check-providers/): which output modes a model
  supports.
- [How to constrain a field's values](/engine/field-constraints/): `maxLength`, `maxItems`, `enum` and the
  rest.
- [What happens when a model is called](/concepts/what-happens-when-a-model-is-called/): outcomes and the
  repair retry.
- [Answer, refusal and unknown are different states](/concepts/answer-refusal-and-unknown/): what the
