...
4. **Choose how the model's rate limits are handled.** OpenRouter puts no request limit on paid models; the
   ...
   next (`auto`, `fixed` or `fail`). [How to connect a model provider](/integrations/model-providers/) has the
...
5. **Prove the choice with a live probe.** `models check --live` sends one small request per output mode and
   ...
   ```bash
   uv run {{CLI_COMMAND}} models check reader --project . --live \
   ...
   For an output with nested objects or long lists, `uv run {{CLI_COMMAND}} models shapes reader --project . --live`
...
The `reader` agent of the [tested snippets](/engine/snippets/) project reads listing photos. Gemini 2.5 Flash-Lite
