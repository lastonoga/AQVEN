...
export function CaseList({ dataset, chosen }: CaseListProps) {
  ...
  useEffect(() => {
    let active = true
    void api.datasets.casesPage(dataset.dataset_id, query || null, null, cursor).then(
        ...
        <div data-case-table-scroll className="max-h-[min(70vh,42rem)] overflow-auto overscroll-contain rounded-lg border border-border">
          <table aria-label={t("caseTitle")} className="w-max min-w-full border-separate border-spacing-0 text-left text-sm">
            <thead className="text-xs text-muted-foreground">
              <tr>
                <th scope="col" className="sticky top-0 left-0 z-30 w-64 min-w-64 max-w-64 border-r border-b border-border bg-muted px-3 py-2 font-medium">{t("caseName")}</th>
              ...
              {rows.map(({ item, fields }) => (
                <tr key={item.name} className={item.name === chosen?.name ? "bg-muted/40" : "bg-background"}>
                  <th scope="row" className={`sticky left-0 z-10 w-64 min-w-64 max-w-64 border-r border-b border-border px-3 py-3 text-left font-normal ${item.name === chosen?.name ? "bg-muted" : "bg-background"}`}>
                    <Link to={ROUTE_PATH.datasets} params={params} search={{ dataset: dataset.dataset_id, case: item.name }} resetScroll={false} className="block min-w-0 break-words font-mono text-xs font-semibold hover:underline">{item.name}</Link>
                  </th>
                  {columns.map((path) => {
