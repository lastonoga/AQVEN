...
const LEGEND_ID = "flow-canvas-legend"

export function Legend({ open, onToggle }: LegendProps) {
  const t = useTranslations("flow.legend")
  const rows = legendRows({
    container: t("container"),
    loopBackEdge: t("loopBackEdge"),
  ...
  return (
    <Collapsible open={open} onOpenChange={onToggle} className="flex flex-col items-end gap-1.75">
      <CollapsibleTrigger asChild>
        <Expander open={open} marker="triangle" label={t("toggle")} controls={LEGEND_ID} />
      </CollapsibleTrigger>
