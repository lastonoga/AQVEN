...
import type { SchemaFlowNode } from "./to-flow"
import { ZOOM_DURATION_MS, zoomedIn, zoomedOut } from "./viewport"

export type ZoomControlProps = { readonly fitOptions: FitViewOptions<SchemaFlowNode> }
...
function ZoomStepButton({ step }: { readonly step: ZoomStep }) {
  const t = useTranslations("schema.zoom")
  const { zoomTo, getZoom } = useReactFlow<SchemaFlowNode>()
...
export function ZoomControl({ fitOptions }: ZoomControlProps) {
  const t = useTranslations("schema.zoom")
  const { fitView } = useReactFlow<SchemaFlowNode>()
