...
Run with a Python environment that has this checkout's aqven package installed:
    python site/scripts/generate_reference.py
    python site/scripts/generate_reference.py --check
"""
...
from aqven.diagnostics import DIAGNOSTIC_TEXTS, DiagnosticCode, SEVERITY_BY_CODE
...
DOCS = ROOT / "src/content/docs/engineering/reference"
SCHEMAS = ROOT / "public/reference/schemas"
GROUPS = {
...
def api_page() -> str:
    ...
    for name in sorted(aqven.__all__):
        ...
        if inspect.isclass(value):
            ...
            methods = [
                (method_name, member)
                for method_name, member in vars(value).items()
...
def openapi_page() -> str:
    document = json.loads((ROOT.parent / "apps/studio/src/api/openapi.json").read_text())
    lines = [
        "---",
