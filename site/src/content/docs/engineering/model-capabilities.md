...
Use declared provider capabilities to prevent invalid assumptions, then run `uv run {{CLI_COMMAND}} models check --project .`. Use `--live` only after credentials and cost policy are ready. [Multimodal Input](/engineering/multimodal-input/) covers media; [Structured Output](/engineering/structured-output/) covers response modes.
